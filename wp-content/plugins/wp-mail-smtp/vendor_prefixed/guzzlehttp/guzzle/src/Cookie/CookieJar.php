...
class CookieJar implements \WPMailSMTP\Vendor\GuzzleHttp\Cookie\CookieJarInterface
{
    ...
    public function __construct(bool $strictMode = \false, array $cookieArray = [])
    {
        ...
        foreach ($cookieArray as $cookie) {
            if (!$cookie instanceof \WPMailSMTP\Vendor\GuzzleHttp\Cookie\SetCookie) {
                $cookie = new \WPMailSMTP\Vendor\GuzzleHttp\Cookie\SetCookie($cookie);
            }
    ...
    public static function fromArray(array $cookies, string $domain) : self
    {
        ...
        foreach ($cookies as $name => $value) {
            $cookieJar->setCookie(new \WPMailSMTP\Vendor\GuzzleHttp\Cookie\SetCookie(['Domain' => $domain, 'Name' => $name, 'Value' => $value, 'Discard' => \true]));
        }
        return $cookieJar;
    }
    /**
     ...
     */
    public static function shouldPersist(\WPMailSMTP\Vendor\GuzzleHttp\Cookie\SetCookie $cookie, bool $allowSessionCookies = \false) : bool
    {
     ...
     */
    public function getCookieByName(string $name) : ?\WPMailSMTP\Vendor\GuzzleHttp\Cookie\SetCookie
    {
    ...
    public function clear(string $domain = null, string $path = null, string $name = null) : void
    {
        if (!$domain) {
            ...
            $this->cookies = \array_filter($this->cookies, static function (\WPMailSMTP\Vendor\GuzzleHttp\Cookie\SetCookie $cookie) use($domain) : bool {
                return !$cookie->matchesDomain($domain);
            });
        } elseif (!$name) {
            $this->cookies = \array_filter($this->cookies, static function (\WPMailSMTP\Vendor\GuzzleHttp\Cookie\SetCookie $cookie) use($path, $domain) : bool {
                return !($cookie->matchesPath($path) && $cookie->matchesDomain($domain));
            });
        } else {
            $this->cookies = \array_filter($this->cookies, static function (\WPMailSMTP\Vendor\GuzzleHttp\Cookie\SetCookie $cookie) use($path, $domain, $name) {
    ...
    public function extractCookies(\WPMailSMTP\Vendor\Psr\Http\Message\RequestInterface $request, \WPMailSMTP\Vendor\Psr\Http\Message\ResponseInterface $response) : void
    {
        if ($cookieHeader = $response->getHeader('Set-Cookie')) {
            foreach ($cookieHeader as $cookie) {
                $sc = \WPMailSMTP\Vendor\GuzzleHttp\Cookie\SetCookie::fromString($cookie);
                if (!$sc->getDomain()) {
     ...
     */
    private function removeCookieIfEmpty(\WPMailSMTP\Vendor\GuzzleHttp\Cookie\SetCookie $cookie) : void
    {
