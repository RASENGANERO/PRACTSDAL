...
//      $("input#chart-end-date-"+block).attr ("value", $(this).data ("end-date"));
  ...
  function configure_tab_0 () {
    ...
    $('#tab-general').tabs();
    $('#ai-general-plugin-settings-tabs').show();
...
//        var nonce = $(this).attr ('nonce');
    ...
    $("#preview-button-"+tab).button ({
      ...
      var sticky_block  = $("input#sticky-"+block).is(":checked") ? 1 : 0;
      var sticky_height = parseInt ($("input#sticky-height-"+block).val ());
...
//      }
  ...
  function configure_website_list () {
    ...
    $("#ai-website-list-table .ai-connect-website").click (function () {
      ...
      var url_parameters = row.hasClass ('read-only') ? '&read-only=1' : '';
      url_parameters = url_parameters + (ai_settings.includes ('ai-settings=1') ? '&ai-settings=1' : '');
      url_parameters = url_parameters + (ai_settings.includes ('ai-show-errors=1') ? '&ai-show-errors=1' : '');
...
//      if (!$("div#statistics-container-" + active_tab).is(':visible')) {
//        $("input#statistics-button-"+active_tab).click ();
//      }
    });
