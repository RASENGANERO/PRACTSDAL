...
//    var ai_debug = false;
    ...
    if (ai_debug) {
      ...
      console.log ('AI ROTATE wrapper', $(rotation_block).attr ('class') + ',', rotate_options.length, 'options');

      var block_wrapping_div = $(rotation_block).closest ('div.' + ai_block_class_def);
      if (block_wrapping_div.length != 0) {
        console.log ('AI ROTATE block', $(block_wrapping_div).attr ('class'));
      }
...
//          var groups = JSON.parse (b64d ($(ai_groups).first ().data ('ai-groups')));
  ...
  //                 var random_index = ai_rotation_seed_block + block_counter;
                   var random_index = ai_rotation_seed_block + block_counter - 1;
...
//          data [1] = random_index + 1;
          ...
          if (timed_rotation && ai_tracking_finished) {
            ...
            adb_show_wrapping_div.addClass ('ai-no-pageview');
          }
...
//          data [1] = random_index + 1;
          ...
          if (timed_rotation && ai_tracking_finished) {
            // Prevent pageview trackign for timed rotations
            wrapping_div.addClass ('ai-no-pageview');
          }
