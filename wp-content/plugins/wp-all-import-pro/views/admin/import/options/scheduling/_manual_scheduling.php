<div style="padding-top:11px;">
    ...
    <div style="<?php if ($post['scheduling_enable'] != 2) { ?> display: none; <?php } ?>" class="manual-scheduling">
        ...
        <h5 style="margin-bottom: 10px; margin-top: 10px; font-size: 14px;"><?php _e('Example Trigger Cron Command'); ?></h5>
        <code style="padding: 10px; border: 1px solid #ccc; display: block; width: 90%;">
		    <?php echo 'wget -q -O - "'.site_url() . '/wp-load.php?import_key=' . $cron_job_key . '&import_id=' . $import_id . '&action=trigger&rand="$RANDOM'; ?>
        </code>
        <h5 style="margin-bottom: 10px; margin-top: 10px; font-size: 14px;"><?php _e('Example Processing Cron Command'); ?></h5>
        <code style="padding: 10px; border: 1px solid #ccc; display: block; width: 90%;">
		    <?php echo 'wget -q -O - "'.site_url() . '/wp-load.php?import_key=' . $cron_job_key . '&import_id=' . $import_id . '&action=processing&rand="$RANDOM'; ?>
        </code>
        </p>
        <p style="margin:0; padding-left: 0;"><?php _e('Read more about manual scheduling'); ?>: <a target="_blank" href="https://www.wpallimport.com/documentation/cron/">https://www.wpallimport.com/documentation/cron/</a></p>
    </div>
</div>
