...
$('body').on('click','a',function(event){let obj=this,href=$(obj).attr('href');if(href=='#modal')
{event.preventDefault();let modal=$(obj).data('modal');open_modal(modal,obj);}
else if(href=='#')
{event.preventDefault();}});$('body').on('click','.modal-open',function(event){event.preventDefault();let obj=this,modal=$(obj).data('modal');open_modal(modal,obj);});$('body').on('click','.ccbd-modal-open',function(event){event.preventDefault();let modal='#ccbd-modal-calc';open_modal(modal,false);});$('body').on('click','.modal-layout, .modal-close',function(event){event.preventDefault();if($(this).parent().attr('id')=='load-modal')
{}
else
