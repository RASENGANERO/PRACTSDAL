...
<article class="post-card post-card--vertical">
    <?php $thumb = get_the_post_thumbnail( $single_post->ID, apply_filters( THEME_SLUG . '_widget_article_normal_thumbnail', THEME_SLUG . '_small' ) ); if ( ! $hide_thumbnail && ! empty( $thumb ) ) : ?>  <div class="post-card__thumbnail">
        ...
        <?php if ( ! $hide_description ) : ?>
            <div class="post-card__description">
                <?php
                    ...
                    if ( apply_filters( THEME_SLUG . '_widget_article_normal_excerpt_strip_tags', true ) ) {
                        $excerpt = strip_tags( $excerpt );
                    }
                    $excerpt = $wpshop_helper->substring_by_word( $excerpt, apply_filters( THEME_SLUG . '_widget_article_normal_excerpt', 50 ) );
                    echo $excerpt;
                ?>
            </div>
        <?php endif ?>
