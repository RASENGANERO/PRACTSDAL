...
$is_show_comments_count = ( ! in_array( 'comments_count', $structure_single_hide ) );
...
<article id="post-<?php the_ID(); ?>" <?php post_class( 'article-post' ); ?>>
    ...
    <?php if ( $thumbnail_type != 'wide' && $thumbnail_type != 'full' && $thumbnail_type != 'fullscreen' ) : ?>
        ...
        <?php if ( $is_show_social_top ) { ?>
            <?php do_action( THEME_SLUG . '_single_before_social_top' ) ?>
            <?php get_template_part( 'template-parts/blocks/social', 'buttons' ) ?>
            <?php do_action( THEME_SLUG . '_single_after_social_top' ) ?>
        <?php } ?>
    ...
    <?php if ( $is_show_meta && ( $is_show_author || $is_show_reading_time || ( $is_show_views && $wpshop_template->get_views() > 0 ) || $is_show_comments_count || $is_show_date || $is_show_date_modified ) ) { ?>
        <div class="entry-meta">
            <?php if ( $is_show_author ) echo '<span class="entry-author"><span class="entry-label">' . __( 'Author', THEME_TEXTDOMAIN ) . '</span> <span itemprop="author">' . get_the_author() . '</span></span>'; ?>
            ...
            <?php if ( $is_show_comments_count ) echo '<span class="entry-comments"><span class="entry-label">' . __( 'Comments', THEME_TEXTDOMAIN ) . '</span> <a href="#comments">' . get_comments_number() . '</a></span>'; ?>
	...
	<?php
					...
					<div class="botform"><?php dynamic_sidebar( 'sidebar-6' ); //уникальный id виджета ?></div>
					<?php endif; ?>
...
<?php echo do_shortcode( $wpshop_core->get_option( 'code_after_content' ) ) ?>
