...
//            $this->copy_views();
        ...
        if ( version_compare( $this->db_version, '1.5.1', '<' ) ) {
            $this->upgrade_1_5_1();
        }
    }
...
//        }
    ...
    /**
     * Perform the 1.5.1 upgrade.
     *
     * @return void
     */
    private function upgrade_1_5_1() {
		$options = get_option( 'reboot_options' );

		// hide for default comments_count on single post
		if ( ! empty( $options['structure_single_hide'] ) ) {
			$options['structure_single_hide'] .= ',comments_count';
		}

		update_option( 'reboot_options', $options );
    }
