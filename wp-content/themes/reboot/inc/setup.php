...
if ( $wpshop_core->get_option( 'header_fixed' ) ) {
    add_action( 'wp_head', function () {
        echo "<script>var fixed_header = 'yes';</script>";
    } );
}
...
if ( $wpshop_core->get_option( 'header_menu_fixed' ) ) {
    add_action( 'wp_head', function () {
 ...
 */
add_filter( 'wpshop_breadcrumbs_home_text', function ( $title ) {
    global $wpshop_core;
    if ( $new_title = $wpshop_core->get_option( 'breadcrumbs_home_text' ) ) {
        return $new_title;
    }
 ...
 */
if ( $wpshop_core->get_option( 'breadcrumbs_single_link' ) ) {
...
if ( $wpshop_core->get_option( 'toc_noindex' ) ) {
    add_filter( 'wpshop_toc_noindex', '__return_true' );
}

if ( ! $wpshop_core->get_option( 'toc_open' ) ) {
...
if ( $wpshop_core->get_option( 'toc_place' ) ) {
    add_filter( 'wpshop_toc_place', '__return_false' );
}

add_filter( 'wpshop_toc_header', function ( $title ) {
    global $wpshop_core;
    if ( $new_title = $wpshop_core->get_option( 'toc_title' ) ) {
        return $new_title;
    }
 ...
 * Comments form title
 */
add_filter( 'reboot_comments_form_title', function ( $title ) {
    global $wpshop_core;
    if ( $new_title = $wpshop_core->get_option( 'comments_form_title' ) ) {
        return $new_title;
    }
 ...
 * Show comment time
 */
if ( ! $wpshop_core->get_option( 'comments_time' ) ) {
    add_filter( 'reboot_comments_show_time', '__return_false' );
}


/**
 * Search page title with results
 */
add_filter( 'reboot_search_with_results_title', function ( $title ) {
    global $wpshop_core;
    if ( $new_title = $wpshop_core->get_option( 'structure_search_with_results_title' ) ) {
        return $new_title;
    }
...
/**
 * Search page title without results
 */
add_filter( 'reboot_search_without_results_title', function ( $title ) {
    global $wpshop_core;
    if ( $new_title = $wpshop_core->get_option( 'structure_search_without_results_title' ) ) {
        return $new_title;
    }
...
/**
 * Search page text without results
 */
add_filter( 'reboot_search_without_results_text', function ( $title ) {
    global $wpshop_core;
    if ( $new_title = $wpshop_core->get_option( 'structure_search_without_results_text' ) ) {
        return $new_title;
    }
 ...
 */
if ( ! $wpshop_core->get_option( 'structure_search_page_show' ) ) {
 ...
 * 404 page title
 */
add_filter( 'reboot_404_title', function ( $title ) {
    global $wpshop_core;
    if ( $new_title = $wpshop_core->get_option( 'structure_404_title' ) ) {
        return $new_title;
    }
 ...
 * 404 page text
 */
add_filter( 'reboot_404_text', function ( $title ) {
    global $wpshop_core;
    if ( $new_title = $wpshop_core->get_option( 'structure_404_text' ) ) {
        return $new_title;
    }
 ...
 * Author social title
 */
add_filter( 'reboot_author_social_title', function ( $title ) {
    global $wpshop_core;
    if ( $new_title = $wpshop_core->get_option( 'author_social_title' ) ) {
        return $new_title;
    }
 ...
 * Related posts title
 */
add_filter( 'reboot_related_title', function ( $title ) {
    global $wpshop_core;
    if ( $new_title = $wpshop_core->get_option( 'related_posts_title' ) ) {
        return $new_title;
    }
 ...
 */
if ( $wpshop_core->get_option( 'content_full_width' ) ) {
 ...
 */
add_filter( 'reboot_social_share_title', function ( $title ) {
    global $wpshop_core;
    if ( $new_title = $wpshop_core->get_option( 'social_share_title' ) ) {
        return $new_title;
    }
 ...
 */
if ( $wpshop_core->get_option( 'single_social_share_title_show' ) ) {
 ...
 */
if ( ! $wpshop_core->get_option( 'page_social_share_title_show' ) ) {
 ...
 */
add_filter( 'reboot_single_rating_title', function ( $title ) {
    global $wpshop_core;
    if ( $new_title = $wpshop_core->get_option( 'single_rating_title' ) ) {
        return $new_title;
    }
 ...
 */
add_filter( 'reboot_page_rating_title', function ( $title ) {
    global $wpshop_core;
    if ( $new_title = $wpshop_core->get_option( 'page_rating_title' ) ) {
        return $new_title;
    }
 ...
 * Rating text
 */
if ( $wpshop_core->get_option( 'rating_text_show' ) ) {
 ...
 */
if ( $wpshop_core->get_option( 'advertising_page_display' ) ) {
 ...
 */
if ( ! empty( $wpshop_core->get_option( 'microdata_publisher_telephone' ) ) ) {
    add_filter( 'wpshop_microdata_publisher_telephone', function () {
        global $wpshop_core;
        return $wpshop_core->get_option( 'microdata_publisher_telephone' );
    } );
}
 ...
 */
if ( ! empty( $wpshop_core->get_option( 'microdata_publisher_address' ) ) ) {
    add_filter( 'wpshop_microdata_publisher_address', function () {
        global $wpshop_core;
        return $wpshop_core->get_option( 'microdata_publisher_address' );
    } );
}
