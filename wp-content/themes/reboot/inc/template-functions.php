...
add_filter( 'abc_pagination/virtual_category/title', function ( $title, $term ) {
    $title = get_term_meta( $term->term_id, 'taxonomy_header', true );
 ...
 */
add_filter( 'get_the_excerpt', 'reboot_remove_toc_from_excerpt' );
function reboot_remove_toc_from_excerpt( $post_excerpt ) {
...
add_action( 'wp_head', 'reboot_preload_thumb_image' );
function reboot_preload_thumb_image() {
    if ( is_single() ) {
        global $post;
        $thumb_url = get_the_post_thumbnail_url( $post, 'full' );
        if ( ! empty( $thumb_url ) ) {
            echo '<link rel="prefetch" href="' . $thumb_url . '" as="image" crossorigin>';
        }
    }
}
